...
sink("ClusteringOutput_2.txt", append=FALSE, split=TRUE)
...
MyKMeans <- kmeans(kData, algorithm="Lloyd", 10, iter.max = 500)
...
lbls <- paste("Cluster #",1:10,":\n",MyKMeans$size)
pie(MyKMeans$size, main = "Pie Chart of Cluster Sizes", col = 1:10, labels = lbls)
...
for (i in 1:length(VAR_LIST)){
  ...
  lines(1:10,meansbycluster, col="red") #add a line to represent centroid
  abline(h=0,lty=2,xlim=c(1,10)) #add a horizontal line at zero
...
write.csv(outFile,file="ClusterContents_2.csv")
