...
sink("ClusteringOutput_3.txt", append=FALSE, split=TRUE)
...
# The third term is the number of clusters we just chose. Here we choose 5. You can choose your own value if you want a different clustering
# The fourth term is the number of iterations we want to run in the algorithm. Here we choose 500. You can choose your own value. 
#    Generally, the more iterations you run in the algorithm, the better results you will have. But more iterations, longer computing time.

MyKMeans <- kmeans(kData, algorithm="Lloyd", 7, iter.max = 500)
...
lbls <- paste("Cluster #",1:7,":\n",MyKMeans$size)
pie(MyKMeans$size, main = "Pie Chart of Cluster Sizes", col = 1:7, labels = lbls)
...
for (i in 1:length(VAR_LIST)){
  ...
  lines(1:7,meansbycluster, col="red") #add a line to represent centroid
  abline(h=0,lty=2,xlim=c(1,7)) #add a horizontal line at zero
...
write.csv(outFile,file="ClusterContents_3.csv")
